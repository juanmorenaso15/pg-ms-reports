...
public class ExportacionIngresosService {
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            if (!pagos.isEmpty()) {
                ...
                for (EventoPago pago : pagos) {
                    table.addCell(
                            new com.itextpdf.layout.element.Cell().add(new Paragraph(pago.getSocioIdentificador())));
                    table.addCell(
                            new com.itextpdf.layout.element.Cell().add(new Paragraph(pago.getMonto().toString())));
                    table.addCell(new com.itextpdf.layout.element.Cell()
                            .add(new Paragraph(pago.getFechaPago().format(DATE_FORMATTER))));
                }
     ...
     */
    public byte[] generarExcelIngresosDiarios(LocalDate fecha, List<EventoPago> pagos, BigDecimal total,
            Long cantidad) {
    ...
    /**
     * Crea un objeto ReporteIngresosDiariosDTO con los datos del reporte de ingresos diarios.
     *
     * @param fecha    La fecha para la cual se desea generar el reporte.
     * @param total    El total de ingresos para la fecha especificada.
     * @param cantidad La cantidad de pagos para la fecha especificada.
    ...
    public byte[] generarPDFIngresosMensuales(ReporteIngresosMensualesDTO dto) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            for (MembresiaIngresoDTO item : dto.getDetalle()) {
                ...
                table.addCell(
                        new com.itextpdf.layout.element.Cell().add(new Paragraph("$" + item.getTotal().toString())));
            }
    ...
    /**
     * Genera un archivo Excel con el reporte de ingresos mensuales.
     *
     * @param dto El objeto ReporteIngresosMensualesDTO que contiene los datos del reporte.
     * @return Un array de bytes que representa el archivo Excel generado.
     */
